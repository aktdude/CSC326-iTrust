...
import edu.ncsu.csc.iTrust2.forms.AdvocateForm;
...
public class APIPersonnelTest {
    ...
    /**
     * Tests PersonnelAPI with advocate functionality
     *
     * @throws Exception
     */
    @Test
    ...
    public void testPersonnelAdvocateAPI () throws Exception {

        final Personnel adv = new Personnel( new UserForm( "adv", "123456", Role.ROLE_ADVOCATE, 1 ) );

        service.save( adv );

        final AdvocateForm personnel = new AdvocateForm();
        ...
        personnel.setState( State.NC.toString() );
        personnel.setZip( "27514" );
        personnel.setMiddleName( "Middle" );
        personnel.setPassword( "testpas" );
        personnel.setPassword2( "testpas" );
        personnel.setNickname( "nickname" );

        // Should be able to update with the new values
        mvc.perform( put( "/api/v1/personnel/advocate/adv" ).with( csrf() ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( personnel ) ) ).andExpect( status().isOk() );

        final Personnel retrieved = (Personnel) service.findByName( "adv" );

        Assertions.assertEquals( "Prag", retrieved.getCity() );
        Assertions.assertEquals( State.NC, retrieved.getState() );
        Assertions.assertEquals( "nickname", personnel.getNickname() );
        Assertions.assertEquals( "Middle", personnel.getMiddleName() );
        Assertions.assertEquals( personnel.getPassword(), personnel.getPassword() );
        Assertions.assertEquals( "testpas", personnel.getPassword() );

        // Edit with wrong url ID should fail
        mvc.perform( put( "/api/v1/personnel/advocate/badhcp" ).with( csrf() ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( personnel ) ) ).andExpect( status().isNotFound() );
