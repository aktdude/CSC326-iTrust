...
public class APIUserController extends APIController {
    ...
    /** Constant for advocate role */
    private static final String              ROLE_ADVOCATE   = "ROLE_ADVOCATE";

    /** All roles */
    private static final List<String>        ALL_ROLES       = List.of( ROLE_ADMIN, ROLE_PATIENT, ROLE_HCP, ROLE_ER,
            ROLE_LABTECH, ROLE_VIROLOGIST, ROLE_OD, ROLE_OPH, ROLE_VACCINATOR, ROLE_BSM, ROLE_ADVOCATE );
    ...
    /**
     * Retrieves and returns the current user
     *
     * @return a user
     */
    ...
    public ResponseEntity getCurrentUser () {
        ...
        loggerUtil.log( TransactionType.VIEW_USER, LoggerUtil.currentUser() );
    ...
    public ResponseEntity deleteUser ( @PathVariable final String id ) {
        ...
        try {
            ...
            if ( user.getRoles().contains( Role.ROLE_ADVOCATE ) ) {
                final List<PatientAdvocateAssignment> list = assignmentService.getAssignmentsByPatientAdvocate( user );
                for ( int i = 0; i < list.size(); i++ ) {
                    try {
                        assignmentService.delete( list.get( i ) );

                    }
                    catch ( final Exception e ) {
                        return new ResponseEntity( errorResponse( "Could not delete " + user.getUsername()
                                + "associations" + " because of " + e.getMessage() ), HttpStatus.BAD_REQUEST );
                    }
                }
                loggerUtil.log( TransactionType.PATIENT_ADVOCATE_ASSIGNMENTS_DELETED, LoggerUtil.currentUser() );
            }
            else if ( user.getRoles().contains( Role.ROLE_PATIENT ) ) {
                final List<PatientAdvocateAssignment> list = assignmentService.getAssignmentsByPatient( user );
                for ( int i = 0; i < list.size(); i++ ) {
                    try {
                        assignmentService.delete( list.get( i ) );
