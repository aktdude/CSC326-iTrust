...
public class APIPersonnelController extends APIController {
     ...
     * @return response
     */
    @PutMapping ( BASE_PATH + "/personnel/advocate/{id}" )
    @PreAuthorize ( "hasAnyRole('ROLE_ADMIN')" )
    public ResponseEntity updateAdvocate ( @PathVariable final String id, @RequestBody final AdvocateForm personnelF ) {
        ...
        if ( ( !fromDb.getRoles().contains( Role.ROLE_ADVOCATE ) ) ) {
            return new ResponseEntity( errorResponse( "The user provided does not have the role of advocate." ),
                    HttpStatus.CONFLICT );
        }
        fromDb.updateAdvocate( personnelF );
    ...
    @PreAuthorize ( "hasAnyRole('ROLE_HCP', 'ROLE_ADMIN', 'ROLE_PATIENT', 'ROLE_VACCINATOR', 'ROLE_BSM', 'ROLE_ADVOCATE')" )
